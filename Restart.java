...
class Restart extends Event {
    ...
    public Event createEvent(String in, long l, int l2, GreenhouseControls c){
	...
	Object instance;
  	try{
		...
		constructor = thermo.getConstructor
                    (long.class, int.class, GreenhouseControls.class);
    ...
    Event powerO = createEvent("PowerOut", 5000, 0, controller);
    Thread pOut = new Thread(powerO);
    pOut.start();
    controller.addThread(pOut);	    
...
//                      controller.addEvent(new Bell(dLay,(int)nRings, this.controller));
			Event bell = createEvent("Bell",dLay, (int)nRings, controller);
